...
def window2_init():
    ...
    l7 = tk.Label(window2, text='年龄', font=(' Microsoft YaHei', 12), width=16, height=2)#,relief=SUNKEN)
    l7.grid(row=3,column=0)
    l8 = tk.Label(window2, text='年级', font=(' Microsoft YaHei', 12), width=16, height=2)#,relief=SUNKEN)
    l8.grid(row=4,column=0)
    l9 = tk.Label(window2, text='专业', font=(' Microsoft YaHei', 12), width=16, height=2)#,relief=SUNKEN)
    l9.grid(row=5,column=0)
    l7 = tk.Label(window2, text='已修课程平均分', font=(' Microsoft YaHei', 12), width=16, height=2)#,relief=SUNKEN)
    l7.grid(row=6,column=0)

    l8 = tk.Label(window2, text='姓名', font=(' Microsoft YaHei', 12), width=4, height=2)#,relief=SUNKEN)
    l8.grid(row=8,column=0)
    l9 = tk.Label(window2, text='学号', font=(' Microsoft YaHei', 12), width=4, height=2)#,relief=SUNKEN)
    ...
    e8 = tk.Entry(window2, font=('Microsoft YaHei', 8))
    e8.grid(row=8,column=1)
    e9 = tk.Entry(window2, font=('Microsoft YaHei', 8))
    e9.grid(row=8,column=3)


    administrator_password = '000000'
    def add_std_data():
        global times2
        one_administrator_password = askstring("Spam", "Input administrator password:",show='*')
        if  one_administrator_password == administrator_password:
            sut_datas.append([e1.get(),e2.get(),e3.get(),e4.get(),e5.get(),e6.get(),e7.get()])
            tkinter.messagebox.showinfo(title='Hi', message='录入成功!')  # 提示信息对话窗
            workbook=xlwt.Workbook()
            worksheet=workbook.add_sheet('test')
            sheet_title=['姓名','性别','学号','年龄','年级','专业','已修课程平均分',]
    ...
    b2.grid(row=9,column=0,columnspan=4)
    ...
    b3.grid(row=11,column=0,columnspan=6)

    area = ('姓名', "性别", "学号", "年龄", "年级", "专业", "已修课程平均分")
    ac = ("姓名", "性别", "学号", "年龄", "年级", "专业", "已修课程平均分")
    dc = ("姓名", "性别", "学号", "年龄", "年级", "专业", "已修课程平均分")
    ...
    tv.grid(row=10,column=0,columnspan=6)
    window2.mainloop()
