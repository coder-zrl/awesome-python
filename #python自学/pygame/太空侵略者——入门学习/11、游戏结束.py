...
# 添加射中的音效
bao_sound=pygame.mixer.Sound('exp.wav')  # 在hit里面调用，bao_sound.play()

# 总分
score=0
font=pygame.font.Font('C:/Windows/Fonts/msyh.ttc',32)  # 微软雅黑
# font=pygame.font.SysFont('freesansblod.ttf',32)  # 表示使用系统自带的字体
def shou_score():
    text=f'score:{score}'
    score_rander=font.render(text,True,(0,255,0))  # 渲染一下字体，True表示使用24位色，元组表示三原色
    screen.blit(score_rander,(10,10))

# 游戏结束
is_over=False
over_font=pygame.font.Font('C:/Windows/Fonts/msyh.ttc',62)
def check_is_over():
    if is_over:
        text = 'Game Over'
        score_rander = font.render(text, True, (0, 255, 0))  # 渲染一下字体，True表示使用24位色，元组表示三原色
        screen.blit(score_rander, (255, 255))
