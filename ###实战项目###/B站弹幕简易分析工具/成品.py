...
class GUI(QWidget):
    ...
    def creat_progress_png(self):
        ...
        file_name = '高能进度条_'+ self.bv_LineEdit.text() +'.png'
        plt.savefig(file_name, dpi=500, bbox_inches='tight')  # 解决图片不清晰，不完整的问题
        plt.show()
    ...
    def creat_top10_png(self):
        ...
        file_name = '弹幕Top10_'+ self.bv_LineEdit.text() +'.png'
        plt.savefig(file_name, dpi=500, bbox_inches='tight')  # 解决图片不清晰，不完整的问题
        plt.show()
    ...
    def creat_type_png(self):
        ...
        if len(barrage_type)==0:
            ...
            file_name = '弹幕类型图_' + self.bv_LineEdit.text() + '.png'
            plt.savefig(file_name, dpi=500, bbox_inches='tight')  # 解决图片不清晰，不完整的问题
            plt.show()
    ...
    def cerat_color_png(self):
        ...
        file_name = '弹幕颜色类型图_' + self.bv_LineEdit.text() + '.png'
        plt.savefig(file_name, dpi=500, bbox_inches='tight')  # 解决图片不清晰，不完整的问题
        plt.show()
    ...
    def creat_wordcloud_png(self):
        ...
        w.generate(laststr)
        file_name = self.bv_LineEdit.text()+'.png'
        w.to_file(file_name)
        QMessageBox.information(self, '提示', '词云_' + file_name  + '已保存', QMessageBox.Yes)
    def save_excel(self):
        ...
        file_name=self.bv_LineEdit.text() +'.xls'
        workbook.save(file_name)
        QMessageBox.information(self, '提示', '表格_'+file_name+'已保存', QMessageBox.Yes)
