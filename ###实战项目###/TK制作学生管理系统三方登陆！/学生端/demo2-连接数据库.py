...
class Student():
    def __init__(self,id,old_password,all_students):
        self.id=id
        ...
        self.cbx_2["values"] = self.subject_list
    ...
    def search_oneself(self):
        ...
        data=[i for i in self.all_students if self.id in i][0]
        for i in range(len(entrys)):
            message=entrys[i].get()
            if message!='':
                # print(message)
                entrys[i].delete(0,tk.END)
            entrys[i].insert(0,data[i])
    ...
    def search_paiming(self):
        ...
        index=self.subject_list.index(subject)+2
        id_score=[]
        for i in self.all_students:
            id_score.append([i[1],i[index]])
        id_score.sort(key=lambda x:eval(x[1]),reverse=True)
        paiming=[i+1 for i in range(len(id_score)) if self.id in id_score[i]][0]
        old_paiming = self.e9.get()
        if old_paiming != '':
            self.e9.delete(0, tk.END)
            # print(data)
        self.e9.insert(0,paiming)
        # print(subject,index)


stu = Student('17074501','17074501',all_students)  # 声明Application对象实例
